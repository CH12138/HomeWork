...
export default class UForm1 extends Component{
    ...
    //getData
    getData = () => {
        axios.get('http://localhost:8080/transdetail/list')
    ...
    onSearchUserName = (value) => {
        ...
        this.setState({
            dataSource: dataSource.filter(item => item.cust_id!== -1),
            loading: false,
        })
    };
    //地址级联选择
    ...
    editClick = (key) => {
        ...
        form.setFieldsValue({
            key: key,
            surname: dataSource[index].surname,
            gender: dataSource[index].gender,
            educa_des: dataSource[index].educa_des,
            mar_des: dataSource[index].mar_des,
            birthday: dataSource[index].birthday,
